...
class GeneExpressionDataset(Dataset):
    ...
    def __getitem__(self, idx=None):
        ...
        if self.scaling_type == 'mean':
            lib = torch.mean(expression, dim=-1) # 1D tensor with the mean mutation count for each sample. Mean mutation count for each mutation type across samples in a batch, says Farhad.
